  ...
  namespace detail
  {
    LLFIO_HEADERS_ONLY_FUNC_SPEC result<size_t> remove_all(directory_handle &&topdirh, LLFIO_V2_NAMESPACE::detail::function_ptr<result<void>(remove_all_callback_reason reason, remove_all_callback_arg arg1, remove_all_callback_arg arg2)> _callback, size_t threads) noexcept
    {
      ...
      try
      {
        ...
        auto enumerate_and_remove = [&](directory_handle &dirh, size_t mylevel) -> result<void> {
...
#ifdef _WIN32
                /* We have a custom implementation for Microsoft Windows, because internally
                file entry works by opening a new HANDLE to the file entry, setting its
        ...
        if(enumerate_and_remove(topdirh, 0))
        {
        ...
        done.store(0, std::memory_order_relaxed);  // threads ought to exit
