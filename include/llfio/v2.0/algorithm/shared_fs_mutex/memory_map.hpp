      ...
      static result<memory_map> fs_mutex_map(const path_handle &base, path_view lockfile) noexcept
      {
        ...
        try
        {
          ...
          if(lockinuse.has_error())
          {
            ...
            lockinuse = ret.lock_file_range(_lockinuseoffset, 1, file_handle::lock_kind::shared);  // inuse shared access, blocking
            if(!lockinuse)
            {
              return std::move(lockinuse).error();
            }
          ...
          */
          OUTCOME_TRY(lockinuse2, ret.lock_file_range(_lockinuseoffset, 1, file_handle::lock_kind::shared));
